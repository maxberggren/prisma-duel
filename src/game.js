...
function drawGizmo() {
  ...
  /* Start it at the nose, clear of the hull and its shield halo. The track is
...
/** Offer a rematch once the field is settled. The controls are hidden, not
    torn out: the next match is started in place, and it needs them back. */
function showRematch() {
  if (ATT.on) return;      // the attract match restarts itself; it has no rematch
  $('orders').classList.add('over');
}
...
function tickClock() {
  ...
  if (left <= 0) {
    ...
    /* Your own orders go in as they stand. The clock running out used to
       throw away the course you had been setting and fly last turn's instead,
       which is a straight line if you had just been turning -- the one thing
       nobody ever means. Stragglers on other machines still hold their last
       course, because that is all this peer knows about them. */
    const me = selfShip();
    if (me && me.alive && !committed && G.moves[me.id] === undefined) commitOrders();
...
function wireNet() {
  ...
  Net.on('chat', ({ peerId, text }) => {
    if (peerId !== Net.selfId() && rematchOnChat(peerId, text)) return;
...
/* ================================================================= rematch
   ...
   put everyone back through the lobby; instead the REMATCH button asks the
   room, the others get a card over their screen with JOIN / BACK TO LOBBY,
   and the moment everyone still in the room has joined, the arbiter starts
   a fresh arena with the same pilots through the ordinary start handshake.
   Whoever leaves simply drops out of the roster, and the rest can still go.
   Practice has no room to ask: REMATCH just restarts against the bots. */
const RM_REQ = '\u0001RMQ';     // {name}
const RM_ACC = '\u0001RMA';     // (no payload)
const RM_NO  = '\u0001RMN';     // (no payload) -- leaving
const RM = { open: false, in: new Set(), asked: false, players: null, seed: 0 };

function rematchReset() {
  RM.open = false; RM.in.clear(); RM.asked = false;
  $('rematch').style.display = 'none';
  $('orders').classList.remove('over');
  // the last match's fire and wreckage do not belong in the next one
  vfx.fire.length = vfx.puffs.length = vfx.sparks.length = 0;
  vfx.glows.length = vfx.wrecks.length = 0;
  shakeAmp = fireShake = hitShake = 0;
}

/** Everyone from the last match who is still in the room -- the pilots a
    rematch would be between, and the set that has to be all-in to start. */
function rematchPool() {
  if (!RM.players) return [];
  const here = new Set(lobbyRoster.map(p => p.peerId));
  return RM.players.filter(p => here.has(p.id));
}

function rematchStatus() {
  const pool = rematchPool();
  const inNames = pool.filter(p => RM.in.has(p.id)).map(p => p.id === Net.selfId() ? 'YOU' : p.name);
  const waiting = pool.filter(p => !RM.in.has(p.id)).map(p => p.name);
  $('rmStatus').textContent = (inNames.length + '/' + pool.length + ' in: ' + (inNames.join(', ') || '—'))
    + (waiting.length ? ' — waiting for ' + waiting.join(', ') : '');
  $('bRmJoin').style.display = RM.in.has(Net.selfId()) ? 'none' : '';
  $('bRmJoin').textContent = 'JOIN REMATCH';
}

/** Show the card. `by` is who asked; null when it is us waiting. */
function rematchShow(by) {
  RM.open = true;
  $('rmT').textContent = 'Rematch';
  $('rmSub').textContent = by ? by + ' WANTS A REMATCH — SAME PILOTS, FRESH ARENA'
                              : 'WAITING FOR THE OTHERS TO JOIN';
  $('rematch').style.display = '';
  rematchStatus();
}

/** Arbiter only: everyone still here is in -> start, through the normal handshake. */
function rematchMaybeStart() {
  if (!NET.live || !Net.isArbiter() || inMatch()) return;
  const pool = rematchPool();
  if (pool.length < 2 || !pool.every(p => RM.in.has(p.id))) return;
  const players = pool.slice().sort((a, b) => a.id - b.id).map(p => ({ id: p.id, name: p.name }));
  RM.seed = (RM.seed * 1103515245 + 12345) >>> 0;
  const seed = (players.reduce((a, p) => (a * 31 + p.id) >>> 0, 20260815) ^ RM.seed) >>> 0;
  const payload = { players, seed, sig: RULES_SIG };
  Net.sendChat(START_TAG + JSON.stringify(payload));
  startMatchFrom(payload);
}

function rematchJoin() {
  if (!NET.live) return;
  RM.in.add(Net.selfId());
  if (!RM.asked) { RM.asked = true; Net.sendChat(RM_REQ + JSON.stringify({ name: selfShip() ? selfShip().name : 'PILOT' })); }
  else Net.sendChat(RM_ACC);
  rematchShow(null);
  rematchMaybeStart();
}

$('oRematch').addEventListener('click', () => {
  if (G.phase !== 'over') return;
  if (!NET.live) { startSolo(((G.state && G.state.seed) || 20260815) + 7919); return; }
  rematchJoin();
});
$('bRmJoin').addEventListener('click', () => rematchJoin());
$('bRmLeave').addEventListener('click', () => {
  if (NET.live) { try { Net.sendChat(RM_NO); Net.disconnect(); } catch (e) {} }
  location.reload();
});

/** Rematch traffic rides the chat channel like the start handshake does. */
function rematchOnChat(peerId, text) {
  if (!NET.live || !RM.players) return false;
  if (text.slice(0, RM_REQ.length) === RM_REQ) {
    let name = 'A PILOT';
    try { name = (JSON.parse(text.slice(RM_REQ.length)).name || name).toUpperCase(); } catch (e) {}
    RM.asked = true; RM.in.add(peerId);
    if (!RM.open) rematchShow(name); else rematchStatus();
    rematchMaybeStart();
    return true;
  }
  if (text.slice(0, RM_ACC.length) === RM_ACC) { RM.in.add(peerId); if (RM.open) rematchStatus(); rematchMaybeStart(); return true; }
  if (text.slice(0, RM_NO.length) === RM_NO) { RM.in.delete(peerId); if (RM.open) rematchStatus(); return true; }
  return false;
}
...
const inMatch = () => !!G.state && !ATT.on && G.phase !== 'over';
function startMatchFrom(payload) {
  ...
  attractStop();
  rematchReset();
  RM.players = players;                       // who a rematch would be between
...
$('bJoin').addEventListener('click', async () => {
  ...
  try {
    ...
    lobbyRoster = roster;
    wireNet();
    Net.on('roster', r => { lobbyRoster = r; refreshLobby(); if (RM.open) { rematchStatus(); rematchMaybeStart(); } });
    Net.on('chat', ({ text }) => {
